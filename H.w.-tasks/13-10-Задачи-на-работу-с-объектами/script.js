...
const personalPlanPeter = {
    ...
    showAgeAndLangs: function(obj){
      return `Мне ${obj.age} и я владею языками: ${obj.skills.languages.map((item)=> item.toUpperCase()).join(' ')}`
    }
};
...
function showProgrammingLangs(plan) {
  return Object.entries(plan.skills.programmingLangs).map(([lang,perc]) => `Язык ${lang} изучен на ${perc}`).join('\n')+'\n'
}
