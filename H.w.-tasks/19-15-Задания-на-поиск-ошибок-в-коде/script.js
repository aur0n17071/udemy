...
function transferWaitors(data) {
    ...
    copy.waitors = restorantData.waitors.slice();
...
function isAverageLunchPriceTrue(fDish, sDish, average) {
    if (Number.parseInt(fDish.price) + Number.parseInt(sDish.price) < Number.parseInt(average)) {
        return 'Цена ниже средней';
    } else {
        return 'Цена выше средней';
    }
...
function transferWaitors(data) {
    ...
    for (let item in data) {
      Array.isArray(data[item]) ? copy[item] = data[item].slice() : copy[item] = data[item];
    }
